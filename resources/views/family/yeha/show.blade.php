    ...
    <section class="section">
        <div class="card">
            ...
            <div class="pt-5">
                ...
                <div class="d-flex">
                    <a href="{{ route ('citizen.yeha.create', $family->id) }} " class="btn btn-success mb-3 ms-auto">Tambahkan Data Agota Keluarga {{$family->kepala_keluarga}}</a>
                </div>
            <div class="table-responsive">
                <table class="table table-striped">
                    ...
                    @foreach ($family->citizen as $citizens)
                        <tr>
                            ...
                            <td>
                                <a href="{{ route ('bpjs.yeha.create', $citizens->id) }} " class="btn btn-success btn-sm">Tambah BPJS</a>
                                <a href="{{ route ('citizen.yeha.show', $citizens->id) }} " class="btn btn-primary btn-sm">Lihat Data</a>
                                <a href="{{ route ('citizen.yeha.edit', $citizens->id) }} " class="btn btn-warning btn-sm">Ubah</a>
                                <form action="{{ route ('citizen.yeha.destroy', $citizens->id) }}" method="POST" style="display:inline">
                                    @csrf
                                    @method('DELETE')
                ...
                <a href="{{route('yeha.index')}}" class="btn btn-secondary">Kembali Ke Daptar Kepala Keluraga</a>
            </div>
        </div>
